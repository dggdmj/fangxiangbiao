...
var Custom = function () {
    ...
    var public_ajax_submit2=function(jel){  //参数（触发的对象）
        ...
        var jump_url = $(obj).attr("data-jump-url");    //页面跳转的url
        Public.Loading();   //loading
        ...
        $(".webuploader_input").each(function(index, el) {
            if($(this).val() != ''){
                ...
                $.post('/manager/PublicFun/get_edit_upload', {ids: $(this).val()}, function(data, textStatus, xhr) {
                    ...
                    if(data['status']){
                        for (var i = 0; i < data['info'].length; i++) {
                            var img_html='<div class="file-item thumbnail lightbox_img upload-state-done"><i class="fa fa-times del_upload_file_style del_upload_file_btn" data-upload-id="'+data['info'][i]['id']+'"></i><a href="'+data['info'][i]['url']+'" target="_blank" title="'+data['info'][i]['title']+'"><img src="'+data['info'][i]['url']+'"></a><div class="info">'+data['info'][i]['title']+'</div></div>';
